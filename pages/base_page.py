...
class BasePage:
    ...
    def add_to_the_basket(self, locator_sourse, locator_target):
        element = self.find_element_with_wait(locator_sourse) # находит элемент, который нужно перетащить
        target = self.find_element_with_wait(locator_target) # находит элемент-приёмник (корзину)
        drag_and_drop(self.driver, element, target) # взятие элемента и перенос его в место назначения 
